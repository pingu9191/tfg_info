...
def main():
            ...
            for lap, t in ret:
                ...
                if models == []:
                    for section in track.sections:
                        ...
                        # Create model
                        model = MyModel.load_model(f"{output_path}/models/model_section_{section.name}.h5")
                        models.append(model)
                    ...
                    if track.sections[-1] == section:
                        ...
                        for lap_section, scalar_value in zip(lap_sections, scalar_values):
                            ...
                            if len(batchs_series[k]) >= BATCH_SIZE:
                                # Train model
                                predictions = models[k].predict([np.array(batchs_series[k]), np.array(batchs_scalar[k])])
                                print("Predictions for section ", track.sections[k].name, ": ", predictions)
                                print("Values: ", label_values)
                                batchs_series[k] = []
                                batchs_scalar[k] = []
                                if k == len(models) - 1:
                                    label_values = []
    ...
    i = 0
    for model in models:
        model.model.save(f"{output_path}/models/model_section_{i}")
